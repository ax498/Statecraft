## 出奇制胜：选择对方一张任意类型卡牌，在对方该类型中取分值最高的一张弃入弃牌堆
extends RefCounted

const CARD_NAME: String = "出奇制胜"
const ABILITY_ID: String = "chuqizhisheng"
...
func get_log_format(card: CardResource) -> String:
	return "[color=#e05050]抽到「出奇制胜」(分值 %d) (可弃对手最高分牌)[/color]" % card.effect_value
...
func effect(card: CardResource, player_index: int, ctx: Dictionary) -> void:
	...
	if bm != null and bm.has_method("_effect_chuqizhisheng"):
		bm.call("_effect_chuqizhisheng", player_index, ctx)
...
func _effect_draw_animation(_card: CardResource, _player_index: int, ctx: Dictionary) -> void:
	...
	tween.tween_property(card_ui, "modulate", Color(1.2, 0.85, 0.85), 0.15).set_trans(Tween.TRANS_SINE)
